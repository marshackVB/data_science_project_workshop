...
feature_lookups = [
    FeatureLookup(
      table_name = f'default.ticket_features_{current_user}',
      feature_names = ['CabinChar', 'CabinMulti', 'Embarked', 'FareRounded', 'Parch', 'Pclass'],
      lookup_key = 'PassengerId'
    ),
    FeatureLookup(
      table_name = f'default.demographic_features_{current_user}',
...
passengers_and_target = (spark.table(f'default.{current_user}_train')
...
model_registry_name = f'feature_store_models_{current_user}'
try:
